...
export function isTimedExercise(name: string): boolean {
  return /\bplank\b|wall sit|hollow hold|dead hang|l-sit|\bisometric\b|\biso hold\b|farmer'?s? (carry|walk)|suitcase carry|jump rope|rope interval|engine round|burpee interval|bike interval|row(er)? interval|sprint interval|\btabata\b/i.test(name)
}
